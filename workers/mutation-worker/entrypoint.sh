...
if [ "$mutation_exists" != "" ]; then
    ...
    mutation-core analyze -f="muts-coinselection-cpp" -c="cmake --build build -j $(nproc) && ./build/src/test/test_bitcoin --run_test=coinselector_tests && ./build/src/test/test_bitcoin --run_test=spend_tests && ./build/test/functional/rpc_psbt.py && ./build/test/functional/wallet_fundrawtransaction.py"
