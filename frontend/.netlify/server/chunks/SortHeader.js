...
class CommonHelper {
  ...
  static isEmpty(value) {
    return value === "" || value === null || value === "00000000-0000-0000-0000-000000000000" || // zero uuid
    value === "0001-01-01 00:00:00.000Z" || // zero datetime
    value === "0001-01-01" || // zero date
    typeof value === "undefined" || Array.isArray(value) && value.length === 0 || CommonHelper.isObject(value) && Object.keys(value).length === 0;
  }
  /**
  ...
  static isFocusable(element) {
    let tagName = element && element.tagName ? element.tagName.toLowerCase() : "";
    return CommonHelper.isInput(element) || tagName === "button" || tagName === "a" || tagName === "details" || element.tabIndex >= 0;
  }
  /**
  ...
  static setByPath(data, path, newValue, delimiter = ".") {
    ...
    for (const part of parts) {
      if (!CommonHelper.isObject(result) && !Array.isArray(result) || !CommonHelper.isObject(result[part]) && !Array.isArray(result[part])) {
        result[part] = {};
      }
      result = result[part];
    }
  ...
  static deleteByPath(data, path, delimiter = ".") {
    ...
    if (parts.length > 0 && (Array.isArray(result) && !result.length || CommonHelper.isObject(result) && !Object.keys(result).length) && (Array.isArray(data) && data.length > 0 || CommonHelper.isObject(data) && Object.keys(data).length > 0)) {
  ...
  static trimQuotedValue(val) {
    if (typeof val == "string" && (val[0] == `"` || val[0] == `'` || val[0] == "`") && val[0] == val[val.length - 1]) {
  ...
  static slugify(str, delimiter = "_", preserved = [".", "=", "-"]) {
    ...
    return str.replace(new RegExp("[" + preserved.join("") + "]", "g"), " ").replace(/[^\w\ ]/gi, "").replace(/\s+/g, delimiter);
  }
  /**
  ...
  static getFileType(filename) {
    if (CommonHelper.hasImageExtension(filename))
      return "image";
    if (CommonHelper.hasDocumentExtension(filename))
      return "document";
    if (CommonHelper.hasVideoExtension(filename))
      return "video";
    if (CommonHelper.hasAudioExtension(filename))
      return "audio";
    return "file";
  }
  /**
  ...
  static extractColumnsFromQuery(selectQuery) {
    ...
    selectQuery = (selectQuery || "").replace(/\([\s\S]+?\)/gm, groupReplacement).replace(/[\t\r\n]|(?:\s\s)+/g, " ");
    const match = selectQuery.match(/select\s+([\s\S]+)\s+from/);
    const expressions = match?.[1]?.split(",") || [];
    const result = [];
    for (let expr of expressions) {
      const column = expr.trim().split(" ").pop();
  ...
  static parseIndex(idx) {
    ...
    const rawColumns = (matches[5] || "").replace(/,(?=[^\(]*\))/gmi, "{PB_TEMP}").split(",");
    for (let col of rawColumns) {
      col = col.trim().replaceAll("{PB_TEMP}", ",");
  ...
  static normalizeSearchFilter(searchTerm, fallbackFields) {
    ...
    searchTerm = isNaN(searchTerm) && searchTerm != "true" && searchTerm != "false" ? `"${searchTerm.replace(/^[\"\'\`]|[\"\'\`]$/gm, "")}"` : searchTerm;
    return fallbackFields.map((f) => `${f}~${searchTerm}`).join("||");
  }
}
const SortHeader = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<th tabindex="0"${add_attribute("title", name, 0)} class="${[
    "col-sort " + escape(classes, true),
    (disable ? "col-sort-disabled" : "") + " " + (sort === "-" + name || sort === "+" + name ? "sort-active" : "") + " " + (sort === "-" + name ? "sort-desc" : "") + " " + (sort === "+" + name ? "sort-asc" : "")
  ].join(" ").trim()}">${slots.default ? slots.default({}) : ``}</th>`;
});
export {
  CommonHelper as C,
  SortHeader as S
