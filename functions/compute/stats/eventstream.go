...
func (p *EventStreamProducer) processPull(relPath string) ([]*cloudwatchlogs.InputLogEvent, error) {
...
func (p *EventStreamProducer) processIssue(relPath string) ([]*cloudwatchlogs.InputLogEvent, error) {
	data, err := os.ReadFile(filepath.Join(p.repoPath, relPath))
	if err != nil {
		return nil, fmt.Errorf("read %s: %w", relPath, err)
	}
